...
package com.yooong.studynotefragment.tflite;
...
import com.yooong.studynotefragment.MainActivity;
...
public class YoloV4Classifier implements Classifier {
    ...
    public static Classifier create(
            ...
            throws IOException {
        Log.d("crush_test","classifier made");
        final YoloV4Classifier d = new YoloV4Classifier();
        Log.d("crush_test","yolo4 classifier made");
        String actualFilename = labelFilename.split("file:///android_asset/")[1];
        InputStream labelsInput = assetManager.open(actualFilename);
        Log.d("crush_test","opened labelFile");
        ...
        br.close();
        Log.d("crush_test","modelFIlename: "+modelFilename);
        Log.d("crush_test","assetManager: "+assetManager.toString());
        try {
            Log.d("crush_test","succeed read label, start configure options");
            Interpreter.Options options = (new Interpreter.Options());
            Log.d("crush_test","made options");
            options.setNumThreads(NUM_THREADS);
            Log.d("crush_test","set number of threads");
            if (isNNAPI) {
                Log.d("crush_test","isNNAPI");
                NnApiDelegate nnApiDelegate = null;
                // Initialize interpreter with NNAPI delegate for Android Pie or above
                Log.d("crush_test","check build version");
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                    Log.d("crush_test","running if statement");
...
//                    options.setUseNNAPI(true);
            ...
            if (isGPU) {
                Log.d("crush_test","isGPU");
                GpuDelegate gpuDelegate = new GpuDelegate();
                options.addDelegate(gpuDelegate);
            }
            Log.d("crush_test","option: "+options.toString());
            d.tfLite = new Interpreter(Utils.loadModelFile(assetManager, modelFilename), options);
        } catch (Exception e) {
            Log.d("crush_test","error ocurred : "+ e.getMessage());
    ...
    private static boolean isNNAPI = true;
